...
def load_pretrained(model, default_cfg, num_classes=1000, in_chans=3, filter_fn=None):
    print('loading pretrained model from {}'.format(default_cfg))
    ...
    if in_chans == 1:
        ...
        conv1_weight = state_dict[conv1_name + '.weight']
        modified_conv1_weight = torch.cat((conv1_weight, conv1_weight.clone().detach()), dim=1)
        state_dict[conv1_name + '.weight'] = modified_conv1_weight       
    elif in_chans != 3:
