...
__all__ = ['resnet50mid', 'resnet101mid']
...
class ResNetMid(nn.Module):
    ...
    def featuremaps(self, x):
        ...
        x = self.relu(x)
        x = self.maxpool(x)
        self.feat_maps['layer0'] = x
        x = self.layer1(x)
        self.feat_maps['layer1'] = x
        x = self.layer2(x)
        self.feat_maps['layer2'] = x
        x = self.layer3(x)
        self.feat_maps['layer3'] = x
        x4a = self.layer4[0](x)
        x4b = self.layer4[1](x4a)
        x4c = self.layer4[2](x4b)
        self.feat_maps['layer4'] = x4c
        return x4a, x4b, x4c

    def batch_feat_maps(self):
    ...
    def forward(self, x):
        ...
        # classifier_logits["id"] = getattr(self, 'attr_id')(v)
        # f = v.detach()
        if self.attr_dims is not None:
